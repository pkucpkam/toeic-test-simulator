...
OUTPUT_SQL = r"f:\Project\english-application\crawl\script-migration\seed_postgres_test.sql"
        ...
        if os.path.exists(BASE_DIR):
            for year in sorted(os.listdir(BASE_DIR)):
                ...
                for test_folder in sorted(os.listdir(year_path), key=lambda x: int(x.replace('test', '')) if 'test' in x else 0):
                    ...
                    # Find full_audio_url from audio dir
                    full_audio_url = "NULL"
                    audio_dir = os.path.join(test_path, "audio")
                    if os.path.exists(audio_dir):
                        audio_files = [f for f in os.listdir(audio_dir) if f.endswith('.mp3')]
                        if audio_files:
                            full_audio_url = f"'{audio_files[0]}'"
